...
export default function AnomalousSequencesPage() {
  ...
  const userId = searchParams.get('id') ?? 'USR-007';
  ...
  useEffect(() => {
    let isMounted = true;
    setIsLoading(true);
    fetchJson<{ user: UserDetail }>(`/api/users/${userId}`)
      .then((data) => {
        if (isMounted) {
          setUser(data.user);
          setIsLoading(false);
  ...
  if (isLoading) {
    return <div className="flex items-center justify-center h-64 text-[var(--on-surface-variant)]">Loading sequences...</div>;
  }

  if (!user) {
    return <div className="flex items-center justify-center h-64 text-[var(--on-surface-variant)]">User not found.</div>;
  }

  const peerMax = Math.max(
    user.peerComparison.exfilGb,
    user.peerComparison.teamMaxGb,
    user.peerComparison.teamAvgGb,
    1
  );
  const userPercent = (user.peerComparison.exfilGb / peerMax) * 100;
  const teamMaxPercent = (user.peerComparison.teamMaxGb / peerMax) * 100;
  const teamAvgPercent = (user.peerComparison.teamAvgGb / peerMax) * 100;
      ...
      <section className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-md glass-panel p-lg rounded-xl">
        <div className="flex items-center gap-md">
          ...
          <div>
            ...
            <p className="font-body-lg text-body-lg text-[var(--on-surface-variant)] mt-1">{user.profile.role} • {user.profile.department}</p>
          </div>
        </div>
                ...
                <div className="space-y-md">
                  <div className="space-y-1">
                    ...
                    <div className="h-4 w-full bg-[var(--surface-variant)] rounded-full overflow-hidden">
                      <div className="h-full bg-[var(--error)] glow-error" style={{ width: `${userPercent}%` }} />
                    </div>
                  </div>
                  <div className="space-y-1">
                    <div className="flex justify-between font-code-sm text-[var(--on-surface)]">
                    ...
                    <div className="h-4 w-full bg-[var(--surface-variant)] rounded-full overflow-hidden">
                      <div className="h-full bg-[var(--tertiary-container)]" style={{ width: `${teamMaxPercent}%` }} />
                    </div>
                  </div>
                  <div className="space-y-1">
                    <div className="flex justify-between font-code-sm text-[var(--on-surface)]">
                    ...
                    <div className="h-4 w-full bg-[var(--surface-variant)] rounded-full overflow-hidden">
                      <div className="h-full bg-[var(--primary-container)]" style={{ width: `${teamAvgPercent}%` }} />
                    </div>
                  </div>
                </div>
              </div>
                ...
                <div key={sequence.id} className="p-md bg-[var(--surface-container-high)] rounded-xl border border-[var(--outline-variant)] flex flex-col md:flex-row md:items-center justify-between gap-md">
                  <div className="flex flex-col gap-2">
                    ...
                    <div className="flex flex-wrap items-center gap-2 mt-1">
                      {sequence.steps.map((step, index) => (
                        <span key={`${sequence.id}-${step}`} className={`px-3 py-1 rounded-full font-code-sm border ${sequence.severity === 'critical' && index === sequence.steps.length - 1 ? 'bg-[var(--error-container)] text-[var(--error)] border-[var(--error)]' : 'bg-[var(--surface-variant)] text-[var(--on-surface)] border-[var(--outline-variant)]'}`}>
                          {step}
                        </span>
                        ...
                        if (index < array.length - 1) {
                          acc.push(
                            <span key={`${sequence.id}-arrow-${index}`} className="material-symbols-outlined text-[var(--on-surface-variant)] text-sm">arrow_forward</span>
                          );
                        }
                        return acc;
                      }, [])}
                  ...
                  <div className="flex items-center gap-lg">
                    <div className="text-right">
                      ...
                      <span className={`font-headline-md font-bold ${sequence.severity === 'critical' ? 'text-[var(--error)] glow-error' : 'text-[var(--tertiary)]'}`}>{sequence.risk}</span>
                    </div>
                  </div>
                </div>
              ))}
